...
def q4nx_fused_linear_projection(
    ...
    trace_ddr_id=7,
    func_prefix="",
    kernel_object="q4nx_fused_linear_projection.o",
    verbose=False,
):
    ...
    if k_chunks > 8:
        raise ValueError("linear projection currently supports at most 2048 K")
    ...
    input_fifo = ObjectFifo(input_ty, name="q4nx_linear_input", depth=1)
    weight_patch_fifos = [
        ObjectFifo(
            weight_patch_ty,
            name=f"q4nx_linear_weight_patch_{patch_idx}",
            depth=1,
        )
    ...
    weight_fifos = [
        ...
        .split(
            ...
            names=[
                f"q4nx_linear_weight_{patch_idx}_{chunk_idx}"
    ...
    out_patch_fifos = [
        ObjectFifo(
            patch_out_ty,
            name=f"q4nx_linear_out_patch_{patch_idx}",
            depth=1,
        )
        for patch_idx in range(output_patches)
    ]
    out_fifos = [
        out_patch_fifos[patch_idx]
        .prod()
        .join(
            [chunk_idx * CHUNK_ROWS for chunk_idx in range(PATCH_CHUNKS)],
            obj_types=[chunk_out_ty] * PATCH_CHUNKS,
            names=[
                f"q4nx_linear_out_{patch_idx}_{chunk_idx}"
                for chunk_idx in range(PATCH_CHUNKS)
            ],
            depths=[1] * PATCH_CHUNKS,
            placement=projection_mem_tile(patch_idx),
    ...
    def core_body(input_fifo, weight_fifo, out_fifo, projection_kernel):
    ...
    sequence_types = [input_ty, weight_stream_ty, output_ty]
    if trace_size > 0:
    ...
    with rt.sequence(*sequence_types) as runtime_args:
        input_l3, weight_l3, output_l3 = runtime_args[:3]
        if trace_size > 0:
