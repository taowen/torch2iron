...
static inline void q4nx_project_chunk(
    int32_t reset_accumulator,
    int32_t k_offset,
    const bfloat16 *__restrict normed_hidden,
    const uint8_t *__restrict chunk,
    bfloat16 *__restrict out)
{
    constexpr int vec_len = Q4NX_GROUP_SIZE;
    for (int32_t out_row = 0; out_row < Q4NX_OUT_CHUNK; out_row += 2)
        chess_prepare_for_pipelining chess_loop_range(1, )
    {
        const bfloat16 *__restrict scales0 =
            reinterpret_cast<const bfloat16 *>(chunk)
            + out_row * Q4NX_GROUPS_PER_ROW;
        const bfloat16 *__restrict zeros0 =
            reinterpret_cast<const bfloat16 *>(chunk + Q4NX_SCALE_BYTES)
            + out_row * Q4NX_GROUPS_PER_ROW;
        const uint4 *__restrict qrow0 =
            reinterpret_cast<const uint4 *>(
                chunk + Q4NX_INT4_OFFSET + out_row * Q4NX_INT4_ROW_BYTES);

        const bfloat16 *__restrict scales1 = scales0 + Q4NX_GROUPS_PER_ROW;
        const bfloat16 *__restrict zeros1 = zeros0 + Q4NX_GROUPS_PER_ROW;
        const uint4 *__restrict qrow1 =
            reinterpret_cast<const uint4 *>(
                chunk + Q4NX_INT4_OFFSET + (out_row + 1) * Q4NX_INT4_ROW_BYTES);
...
void q4nx_fused_qkv_projection_patch(
    int32_t reset_accumulator,
    int32_t k_offset,
    const bfloat16 *__restrict normed_hidden,
    const uint8_t *__restrict weight_chunk,
    bfloat16 *__restrict qkv_out)
{
    event0();
    ::aie::set_rounding(aie::rounding_mode::conv_even);
    q4nx_project_patch(
    ...
    q4nx_project_patch(
        ...
        qkv_out + Q4NX_OUT_ROWS);
    q4nx_project_patch(
        reset_accumulator,
        k_offset,
        normed_hidden,
        weight_chunk + 2 * Q4NX_PATCH_BYTES,
        qkv_out + 2 * Q4NX_OUT_ROWS);
    event1();
}

void q4nx_fused_projection_patch(
    int32_t reset_accumulator,
    int32_t k_offset,
    const bfloat16 *__restrict normed_hidden,
    const uint8_t *__restrict weight_chunk,
...
void q4nx_fused_projection_chunk(
    int32_t reset_accumulator,
    int32_t k_offset,
    const bfloat16 *__restrict normed_hidden,
    const uint8_t *__restrict weight_chunk,
...
void q4nx_fused_projection_chunk_full(
    const bfloat16 *__restrict normed_hidden,
    const uint8_t *__restrict weight_chunks,
    bfloat16 *__restrict out)
{
    event0();
    ::aie::set_rounding(aie::rounding_mode::conv_even);
    for (int32_t k_offset = 0; k_offset < Q4NX_FULL_IN_FEATURES;
         k_offset += Q4NX_IN_CHUNK)
        chess_prepare_for_pipelining chess_loop_range(1, )
    {
        q4nx_project_chunk(
            k_offset == 0,
            ...
            normed_hidden,
            weight_chunks
                + (k_offset / Q4NX_IN_CHUNK) * Q4NX_CHUNK_BYTES,
            out);
    }
    event1();
}

void q4nx_residual_add_patch(
    const bfloat16 *__restrict projected,
    const bfloat16 *__restrict residual,
    int32_t residual_offset,
    bfloat16 *__restrict out)
{
    event0();
    constexpr int vec_len = 16;
    ::aie::set_rounding(aie::rounding_mode::conv_even);
    for (int32_t idx = 0; idx < Q4NX_OUT_ROWS; idx += vec_len)
        chess_prepare_for_pipelining chess_loop_range(1, )
    {
        aie::vector<bfloat16, vec_len> projected_vec =
            aie::load_v<vec_len>(projected + idx);
        aie::vector<bfloat16, vec_len> residual_vec =
            aie::load_v<vec_len>(residual + residual_offset + idx);
        aie::vector<bfloat16, vec_len> out_vec =
            aie::add(projected_vec, residual_vec);
        aie::store_v(out + idx, out_vec);
    }
    event1();
}
