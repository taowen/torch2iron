...
@dataclass
class Q4NXFusedUpGateProjection(MLIROperator):
    """Fused RMSNorm + up/gate projection over Q4NX output patches."""
    ...
    @property
    def up_gate_patch_stream_bytes(self) -> int:
        return self.output_patches * 2 * self.patch_bytes
    ...
    def _kernel_object_name(self) -> str:
        ...
        return (
            "q4nx_fused_rms_up_gate_projection_"
    ...
    def get_arg_spec(self):
        return [
            ...
            AIERuntimeArgSpec("in", (self.in_features,), dtype=bfloat16),
            AIERuntimeArgSpec("in", (self.up_gate_patch_stream_bytes,), dtype=np.uint8),
            AIERuntimeArgSpec(
                "out",
                (self.output_patches * 2 * self.out_rows,),
                dtype=bfloat16,
            ),
        ]
