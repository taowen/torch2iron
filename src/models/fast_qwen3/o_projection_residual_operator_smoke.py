...
"""Run fast Qwen3 Q4NX o_proj block with residual add in one ELF."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run fast_qwen3 Q4NX o_proj block plus residual add"
    )
    parser.add_argument("model_dir", help="Qwen3 model directory or parent directory")
    parser.add_argument("--artifact-dir", default=None)
    parser.add_argument(
        "--build-dir",
        default="build/fast_qwen3_o_projection_residual_smoke",
    )
...
def _weight_stream(
    ...
    if not 1 <= output_patches <= max_patches:
...
def main() -> None:
    ...
    fused_op = FusedMLIROperator(
        ...
        input_args=["attention_context", "residual_block"],
        output_args=["residual_out"],
    ...
    actual = fused.get_buffer("residual_out").torch_view().view_as(expected)
    ...
    if max_abs_error > args.abs_tol:
        raise AssertionError(
            f"o_proj residual max_abs_error={max_abs_error} exceeds {args.abs_tol}"
        )
