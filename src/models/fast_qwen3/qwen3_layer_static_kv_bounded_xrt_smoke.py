...
"""Run the bounded static-KV attention contract and compare context output."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run bounded static-KV attention")
...
def _zero_query_reference(
    ...
    value_plane_offsets = (
        PLANES[1].base_bytes // 2,
        PLANES[1].base_bytes // 2,
        PLANES[3].base_bytes // 2,
        PLANES[3].base_bytes // 2,
    )
    for group in range(8):
        pair = group // 2
        half = group % 2
        plane_offset = value_plane_offsets[pair]
        values = []
        for tile in range(tile_count):
            valid_rows = min(16, attend_seq_len - tile * 16)
            offset = plane_offset + tile * 4096 + half * 2048
...
def main() -> None:
    ...
    actual = _bf16_from_u16(context.data).view_as(expected)
    ...
    if max_abs_error > args.abs_tol:
        raise AssertionError(
            f"bounded static-KV attention max_abs_error={max_abs_error} exceeds {args.abs_tol}"
        )
