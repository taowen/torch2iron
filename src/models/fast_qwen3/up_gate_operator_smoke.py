...
"""Compile and run the fast Qwen3 fused up/gate Q4NX patch operator."""
...
from models.fast_qwen3.operators.q4nx_fused_up_gate_projection import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the fast_qwen3 Q4NX up/gate patch operator"
    )
    parser.add_argument("model_dir", help="Qwen3 model directory or parent directory")
    parser.add_argument("--artifact-dir", default=None)
    parser.add_argument("--build-dir", default="build/fast_qwen3_up_gate_patch_smoke")
    parser.add_argument("--trace-dir", default="build_trace/fast_qwen3_up_gate_patch")
...
def main() -> None:
    ...
    for projection_idx, projection in enumerate(("up", "gate")):
    ...
    up_gate_stream = b"".join(chunks)
    up_gate_weight = torch.from_numpy(
        np.frombuffer(up_gate_stream, dtype=np.uint8).copy()
    )
    ...
    fused_op = FusedMLIROperator(
        name="fast_qwen3_up_gate_patch",
        runlist=[(op, "hidden", "norm_weight", "up_gate_weight", "up_gate_out")],
        input_args=["hidden"],
        output_args=["up_gate_out"],
        external_args={
        ...
        trace_file=trace_dir / "up_gate_patch.trace.txt",
        trace_json_file=trace_dir / "up_gate_patch.trace.json",
        trace_op_index=0,
        trace_ddr_id=4,
        context=context,
    ).compile()
    ...
    fused.get_buffer("up_gate_weight").torch_view()[:] = up_gate_weight.flatten()
    fused.mark_buffer_dirty("input")
    fused.mark_buffer_dirty("norm_weight")
    fused.mark_buffer_dirty("up_gate_weight")
    fused.norm_weight_buffer.to("npu")
    fused.up_gate_weight_buffer.to("npu")
    ...
    actual = fused.get_buffer("up_gate_out").torch_view().view(
        args.output_patches,
        2,
        Q4NX_PATCH_OUT_ROWS,
    )
        ...
        json.dumps(
            {
                ...
                "up_gate_patch_bytes": first_patch_bytes,
                "up_gate_stream_bytes": int(up_gate_weight.numel()),
                "rms_norm_epsilon": rms_norm_epsilon,
