...
"""Run current-aware attention directly into one o_proj Q4NX block."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run fast_qwen3 attention through one o_proj block"
    )
    parser.add_argument("model_dir", help="Qwen3 model directory or parent directory")
    parser.add_argument("--artifact-dir", default=None)
    parser.add_argument("--build-dir", default="build/fast_qwen3_attention_o_projection")
...
def main() -> None:
    ...
    if args.o_block < 0 or args.o_block >= max_o_blocks:
        raise ValueError(f"o-block must be in [0, {max_o_blocks - 1}]")
    o_weight, o_patch_views = _linear_weight_stream(
        store,
        o_prefix,
        args.o_block * o_block_patches,
        o_block_patches,
    )
    ...
    o_proj_op = Q4NXFusedLinearProjection(
        in_features=o_spec.in_features,
        output_patches=o_block_patches,
        context=context,
    )
    fused_op = FusedMLIROperator(
        name="fast_qwen3_attention_o_projection",
        runlist=[
            (attn_op, "q_current", "packet_cache", "attn_context"),
            (o_proj_op, "attn_context", "o_weight", "o_proj"),
        ],
        input_args=["q_current"],
        output_args=["o_proj"],
        external_args={"kv_cache": ["packet_cache"], "o_weight": ["o_weight"]},
    ...
    actual_o = fused.get_buffer("o_proj").torch_view().view_as(expected_o)
    ...
    if max_o_error > args.abs_tol:
        raise AssertionError(f"attention_o_proj max_abs_error={max_o_error}")
        ...
        json.dumps(
            {
                ...
                "o_proj_max_abs_error": max_o_error,
                "o_proj_mean_abs_error": float(o_error.mean().item()),
                "o_proj_shape": list(actual_o.shape),
