...
they encode the current IRON buffer layout.
"""
...
def _grouped_query_attention_forward(
    ...
    # As a result of the attention projections, the queries, keys and values
    # for each head are interspersed with each other. Transpose so that heads
    # are consecutive for attention computation.
    ...
    # Step 4: Append newly computed keys/values to the cache.
    keys_cache = torch.cat([keys_cache, keys], dim=2)
    values_cache = torch.cat([values_cache, values], dim=2)
    keys = keys_cache
    values = values_cache

    # Step 5: Repeat values for grouped attention.
    ...
    scores = (
        # to_torch() syncs device->host; torch_view() would not sync here.
...
def _swiglu_ffn_forward(runner, layer_idx):
    aie_ops = runner.aie_ops
    aie_buffers = runner.aie_buffers
...
def prefill_forward_pass(runner, state):
    ...
    if config.n_layers != EXPECTED_PREFILL_LAYERS:
        raise ValueError(
            f"generated prefill graph expects {EXPECTED_PREFILL_LAYERS} layers, "
