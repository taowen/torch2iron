...
MLIR="${REPO_ROOT}/build/fast_qwen3_qwen3_layer_static_kv_full_two_stage_attention_bounded_contract.mlir"
XCLBIN="${REPO_ROOT}/build/fast_qwen3_qwen3_layer_static_kv_full_two_stage_attention_bounded_contract_run.xclbin"
INSTS="${REPO_ROOT}/build/fast_qwen3_qwen3_layer_static_kv_full_two_stage_attention_bounded_contract_run.bin"

"${REPO_ROOT}/scripts/build_fast_qwen3_plane_attention_kernel_object.sh"
"${REPO_ROOT}/scripts/run_fast_qwen3_layer_static_kv_full_two_stage_attention_bounded_contract_smoke.sh" \
  --attend-seq-len 128 \
  --tile-size 16 \
...
exec timeout "${TIMEOUT_SECONDS}s" "${IRON_PYTHON}" -m models.fast_qwen3.qwen3_layer_static_kv_bounded_xrt_smoke \
  --xclbin "${XCLBIN}" \
  --insts "${INSTS}" \
  --attend-seq-len 128 \
  "$@"
